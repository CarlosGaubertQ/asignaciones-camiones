...
if ($datosM['query'] == 1) {
  ...
  $resultado = $mysqli->query('SELECT MANTENCION.ID_MANTENCION, EQUIPO.ID_EQUIPO, EQUIPO.NOMBRE_EQUIPO AS "NOMBRE_EQUIPO", CONCAT("Perfil ", 
  ...
  INNER JOIN PERFIL_EQUIPO ON PERFIL_EQUIPO.ID_PERFIL_EQUIPO = EQUIPO.ID_PERFIL_EQUIPO WHERE ELIMINAR_MANTENCION IS NULL AND MANTENCION.FECHAFIN_MANTENCION IS NULL AND MANTENCION.DIAGNOSTICOTECNICO_MANTENCION IS NULL AND SEDE.NOMBRE_SEDE = "' . $datosM['sede'] . '" ');
  ...
  while ($fila = $resultado->fetch_assoc()) {
    ...
    $arr["data"][$i]['ID_EQUIPO'] = $fila['ID_EQUIPO'];
  ...
  echo json_encode($arr);
    ...
    $resultado = "UPDATE MANTENCION SET DIAGNOSTICOTECNICO_MANTENCION = ?, FECHAFIN_MANTENCION = ? WHERE ID_MANTENCION = ?";
    $statement = $mysqli->prepare($resultado);
    $statement->bind_param('ssi', $datosM['DIAGNOSTICOTECNICO_MANTENCION'], $datosM['FECHAFIN_MANTENCION'], $datosM['ID_MANTENCION']);
    $arr = array();
    ...
    $stmt2 = $mysqli->prepare('UPDATE EQUIPO SET ID_ESTADO = 2 WHERE ID_EQUIPO=' . $datosM['ID_EQUIPO'] . '');
    $stmt2->execute();
