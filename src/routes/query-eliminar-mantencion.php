...
if ($datosM['query'] == 1) {
  ...
  INNER JOIN PERFIL_EQUIPO ON PERFIL_EQUIPO.ID_PERFIL_EQUIPO = EQUIPO.ID_PERFIL_EQUIPO WHERE ELIMINAR_MANTENCION IS NULL AND SEDE.NOMBRE_SEDE = "' . $datosM['sede'] . '" ');
  ...
  echo json_encode($arr);
    ...
    $resultado = "UPDATE MANTENCION SET ELIMINAR_MANTENCION = ? WHERE ID_MANTENCION = ?";
    $statement = $mysqli->prepare($resultado);
    $statement->bind_param('ii',  $datosM['ELIMINAR_MANTENCION'], $datosM['ID_MANTENCION']);
    $arr = array();
